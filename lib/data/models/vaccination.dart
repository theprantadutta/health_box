...
class Vaccinations extends Table {
  ...
  TextColumn get recordType => text().withDefault(const Constant('vaccination')).named('record_type')();
  ...
  TextColumn get vaccineName => text().withLength(min: 1, max: 100).named('vaccine_name')();
  TextColumn get manufacturer => text().nullable().named('manufacturer')();
  TextColumn get batchNumber => text().nullable().named('batch_number')();
  DateTimeColumn get administrationDate => dateTime().named('administration_date')();
  ...
  BoolColumn get isComplete => boolean().withDefault(const Constant(false)).named('is_complete')();
