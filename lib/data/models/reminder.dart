...
class Reminders extends Table {
  ...
  TextColumn get recordId =>
      text().nullable().named('record_id')(); // Reference to medical record
  TextColumn get medicationId => text().nullable().named(
    'medication_id',
  )(); // Optional reference to medication
  TextColumn get type => text().named(
    'type',
  )(); // Type of reminder (medication, appointment, etc.)
  ...
  TextColumn get daysOfWeek =>
      text().nullable().named('days_of_week')(); // JSON array
  TextColumn get timeSlots =>
      text().nullable().named('time_slots')(); // JSON array
  BoolColumn get isActive =>
      boolean().withDefault(const Constant(true)).named('is_active')();
  DateTimeColumn get lastSent => dateTime().nullable().named('last_sent')();
  DateTimeColumn get nextScheduled =>
      dateTime().nullable().named('next_scheduled')();
  IntColumn get snoozeMinutes =>
      integer().withDefault(const Constant(15)).named('snooze_minutes')();
