...
class LabReports extends Table {
  ...
  TextColumn get recordType =>
      text().withDefault(const Constant('lab_report')).named('record_type')();
  ...
  TextColumn get testName =>
      text().withLength(min: 1, max: 100).named('test_name')();
  ...
  TextColumn get orderingPhysician =>
      text().nullable().named('ordering_physician')();
  TextColumn get labFacility => text().nullable().named('lab_facility')();
  TextColumn get testStatus => text().named('test_status')();
  DateTimeColumn get collectionDate =>
      dateTime().nullable().named('collection_date')();
  BoolColumn get isCritical =>
      boolean().withDefault(const Constant(false)).named('is_critical')();
