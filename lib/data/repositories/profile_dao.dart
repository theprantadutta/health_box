...
class ProfileDao {
  ...
  Future<bool> updateProfile(String id, FamilyMemberProfilesCompanion profile) async {
    ...
    final rowsAffected = await (_database.update(_database.familyMemberProfiles)
          ..where((p) => p.id.equals(id)))
        .write(updatedProfile);
  ...
  Future<bool> deleteProfile(String id) async {
    final rowsAffected = await (_database.update(_database.familyMemberProfiles)
          ..where((p) => p.id.equals(id)))
        .write(FamilyMemberProfilesCompanion(
  ...
  Future<bool> permanentlyDeleteProfile(String id) async {
    final rowsAffected = await (_database.delete(_database.familyMemberProfiles)
          ..where((p) => p.id.equals(id)))
        .go();
  ...
  Future<List<FamilyMemberProfile>> getProfilesByAgeRange(int minAge, int maxAge) async {
    ...
    return await (_database.select(_database.familyMemberProfiles)
          ...
          ..orderBy([
            (profile) => OrderingTerm(expression: profile.dateOfBirth, mode: OrderingMode.desc),
          ]))
        .get();
  }
  ...
  Future<List<FamilyMemberProfile>> getRecentlyUpdatedProfiles({int limit = 10}) async {
    return await (_database.select(_database.familyMemberProfiles)
          ..where((profile) => profile.isActive.equals(true))
          ..orderBy([
            (profile) => OrderingTerm(expression: profile.updatedAt, mode: OrderingMode.desc),
          ])
          ..limit(limit))
        .get();
  }
  ...
  Future<bool> profileExists(String id) async {
    final count = await (_database.selectOnly(_database.familyMemberProfiles)
          ..addColumns([_database.familyMemberProfiles.id.count()])
          ..where(_database.familyMemberProfiles.id.equals(id) & 
  ...
  Future<List<FamilyMemberProfile>> getProfilesWithInsurance() async {
    return await (_database.select(_database.familyMemberProfiles)
          ..where((profile) => 
              profile.isActive.equals(true) & 
              profile.insuranceInfo.isNotNull()))
        .get();
  }
