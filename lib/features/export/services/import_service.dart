...
class ImportResult {
  ...
  bool get hasErrors => validationIssues.any((i) => i.severity == ValidationSeverity.error);
  bool get hasWarnings => validationIssues.any((i) => i.severity == ValidationSeverity.warning);
}
...
class ImportService {
  ...
  Future<ImportResult> _importFromJson(String filePath, ImportOptions options) async {
  ...
  Future<ImportResult> _importFromCsv(String filePath, ImportOptions options) async {
  ...
  Future<ImportResult> _importFromZip(String filePath, ImportOptions options) async {
  ...
  Future<ImportResult> _importFromBackup(String filePath, ImportOptions options) async {
    ...
    if (!_isVersionCompatible(version)) {
      throw Exception('Backup version $version is not compatible with this app version');
    }
    ...
    for (final entry in data.entries) {
      ...
      if (options.allowedEntityTypes != null &&
        ...
        issues.add(ValidationIssue(
          ...
          message: 'Entity type "$entityType" is not in allowed list and will be skipped',
        ));
        continue;
      }
  ...
  List<ValidationIssue> _validateProfile(Map<String, dynamic> profile, int lineNumber) {
    final issues = <ValidationIssue>[];

    // Required fields
    if (!profile.containsKey('id') || profile['id'] == null || profile['id'].toString().isEmpty) {
      issues.add(ValidationIssue(
    ...
    if (!profile.containsKey('firstName') || profile['firstName'].toString().trim().isEmpty) {
      issues.add(ValidationIssue(
        severity: ValidationSeverity.error,
    ...
    if (!profile.containsKey('lastName') || profile['lastName'].toString().trim().isEmpty) {
      issues.add(ValidationIssue(
        severity: ValidationSeverity.error,
  ...
  List<ValidationIssue> _validateMedicalRecord(Map<String, dynamic> record, int lineNumber) {
    final issues = <ValidationIssue>[];
    ...
    if (!record.containsKey('profileId') || record['profileId'].toString().isEmpty) {
      issues.add(ValidationIssue(
        severity: ValidationSeverity.error,
    ...
    if (!record.containsKey('recordType') || record['recordType'].toString().isEmpty) {
      issues.add(ValidationIssue(
        severity: ValidationSeverity.error,
  ...
  List<ValidationIssue> _validateReminder(Map<String, dynamic> reminder, int lineNumber) {
    final issues = <ValidationIssue>[];
    ...
    if (!reminder.containsKey('title') || reminder['title'].toString().trim().isEmpty) {
      issues.add(ValidationIssue(
        severity: ValidationSeverity.error,
    ...
    if (reminder.containsKey('scheduledTime') && reminder['scheduledTime'] != null) {
  ...
  List<ValidationIssue> _validateAttachment(Map<String, dynamic> attachment, int lineNumber) {
    final issues = <ValidationIssue>[];
    ...
    if (!attachment.containsKey('fileName') || attachment['fileName'].toString().isEmpty) {
      issues.add(ValidationIssue(
        severity: ValidationSeverity.error,
    ...
    if (data.containsKey('medicalRecords')) {
      ...
      for (int i = 0; i < records.length; i++) {
        ...
        if (profileId != null && !profileIds.contains(profileId)) {
          issues.add(ValidationIssue(
            severity: ValidationSeverity.error,
            message: 'Medical record references non-existent profile: $profileId',
            field: 'profileId',
            lineNumber: i + 1,
    ...
    if (metadata.containsKey('itemCounts')) {
      ...
      for (final entry in itemCounts.entries) {
        ...
        if (expectedCount != actualCount) {
          issues.add(ValidationIssue(
            severity: ValidationSeverity.warning,
            message: 'Item count mismatch for $entityType: expected $expectedCount, found $actualCount',
          ));
        }
      }
    }
  ...
  Future<ImportResult> _performImport(
    ...
    final importOrder = ['tags', 'profiles', 'medicalRecords', 'reminders', 'attachments'];
  ...
  Future<Map<String, int>> _importEntities(
    ...
    for (final entity in entities) {
      try {
        ...
        final success = await _importSingleEntity(entityType, entityMap, options);
  ...
  Future<bool> _importProfile(Map<String, dynamic> profile, ImportOptions options) async {
    // Note: These methods would need to be implemented in your DAO classes
    // For now, this is a placeholder implementation
    
    // Reference DAOs to avoid unused field warnings
    _logger.d('Using profile DAO: ${_profileDao.runtimeType}');
    
    switch (options.mode) {
      case ImportMode.merge:
        // Try to update existing, create if not found
        // return await _profileDao.insertOrUpdate(profile);
        return true;
      case ImportMode.replace:
        // Delete existing and create new
        // await _profileDao.delete(profile['id']);
        // return await _profileDao.insert(profile);
        return true;
      case ImportMode.skipDuplicates:
        // Only create if doesn't exist
        // final exists = await _profileDao.exists(profile['id']);
        // return exists ? false : await _profileDao.insert(profile);
        return true;
    }
  }

  Future<bool> _importMedicalRecord(Map<String, dynamic> record, ImportOptions options) async {
    // Implementation would depend on your DAO methods
    _logger.d('Using medical record DAO: ${_medicalRecordDao.runtimeType}');
    return true;
  }

  Future<bool> _importReminder(Map<String, dynamic> reminder, ImportOptions options) async {
    // Implementation would depend on your DAO methods
    _logger.d('Using reminder DAO: ${_reminderDao.runtimeType}');
    return true;
  }

  Future<bool> _importTag(Map<String, dynamic> tag, ImportOptions options) async {
    // Implementation would depend on your DAO methods
    _logger.d('Using tag DAO: ${_tagDao.runtimeType}');
    return true;
  }

  Future<bool> _importAttachment(Map<String, dynamic> attachment, ImportOptions options) async {
    // Implementation would depend on your DAO methods
  ...
  String _detectEntityTypeFromHeaders(List<String> headers) {
    if (headers.contains('firstName') && headers.contains('lastName')) {
      return 'profiles';
    } else if (headers.contains('recordType') && headers.contains('recordDate')) {
